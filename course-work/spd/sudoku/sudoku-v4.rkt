...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname sudoku-v4) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (valid-board? bd)
  (local [(define (valid-unit? unit)
            (not (has-duplicates? (unit->lov unit))))

          (define (unit->lov unit)
            (filter number? (map (λ (e) (read-square bd e)) unit)))
          
          (define (has-duplicates? lov)
            (cond [(empty? lov) false]
                  [else (or (member (first lov) (rest lov))
                            (has-duplicates? (rest lov)))]))]
